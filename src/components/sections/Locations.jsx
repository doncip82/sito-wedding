  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col gap-[.6rem]">
        <span className="text-[.5rem] font-light tracking-[.24em] uppercase text-white/38">
          {venue.location}
        </span>
        ...
        <p className="text-[.63rem] font-light tracking-[.06em] leading-[1.95]
          text-white group-hover:text-white transition-colors duration-500 max-w-[58ch]"
      ...
      {/* Right col — thumbnail + link */}
      <div className="hidden sm:flex flex-col items-end gap-3 pt-[.15rem]">
        {/* Thumbnail placeholder — revealed on hover */}
        <div className="w-[88px] h-[64px] overflow-hidden flex-shrink-0
          opacity-0 scale-[.93] group-hover:opacity-100 group-hover:scale-100
          transition-all duration-500">
          {/* Replace with: <img src={`/images/${venue.id}.jpg`} alt={venue.name} className="w-full h-full object-cover" /> */}
          <div className="w-full h-full bg-[#232B3A]" />
        </div>
        <a href="/#contact"
          className="text-[.5rem] font-light tracking-[.2em] uppercase no-underline
            text-white/15 border-b border-transparent pb-[2px] white-space-nowrap
...
export default function Locations() {
  ...
  return (
    <section id="locations" aria-labelledby="loc-title"
      className="bg-ink overflow-hidden px-[clamp(1.5rem,6vw,5rem)] py-[clamp(5rem,12vw,10rem)]">
